...
BEGIN_MESSAGE_MAP(CDialogWhria, CDialog)
	//{{AFX_MSG_MAP(CDialogWhria)
	ON_WM_ENTERIDLE()
...
CDialogWhria::CDialogWhria(UINT nIDTemplate,CWnd* pParentWnd)
...
BOOL CDialogWhria::InitDialogEx(BOOL bToolTips /*= FALSE*/, BOOL bStatusBar /*= FALSE*/, UINT *pIndicators /*= NULL*/, UINT nIndicators /*= 0*/, UINT uiToolBar /*= 0*/, DWORD dwToolBarStyle /*= TBSTYLE_FLAT | WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC*/)
{
	if (bToolTips) {
		EnableToolTips(TRUE);
...
void CDialogWhria::PostNcDestroy()
{
	if (!m_bKillItself) {
...
void CDialogWhria::OnStatusBarCheck() 
{
...
void CDialogWhria::OnUpdateStatusBarMenu(CCmdUI* pCmdUI) 
{
	ASSERT(pCmdUI->m_nID == ID_VIEW_STATUS_BAR);
	pCmdUI->SetCheck(m_CSBCtrlStatus.IsWindowVisible());
}

void CDialogWhria::OnUpdateKeyIndicator(CCmdUI* pCmdUI)
{
	UINT nVK;
...
void CDialogWhria::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu) 
{
...
LRESULT CDialogWhria::OnSetMessageString(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CDialogWhria::OnPopMessageString(WPARAM wParam, LPARAM lParam)
{
...
void CDialogWhria::OnEnterIdle(UINT nWhy, CWnd* pWho) 
{
...
void CDialogWhria::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu) 
{
...
void CDialogWhria::OnToolBarCheck() 
{
...
BOOL CDialogWhria::OnToolTipText(UINT, NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CDialogWhria::OnUpdateToolBarMenu(CCmdUI* pCmdUI) 
{
	ASSERT(pCmdUI->m_nID == ID_VIEW_TOOLBAR);
	pCmdUI->SetCheck(m_CSToolBar.IsWindowVisible());
}

void CDialogWhria::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bResetPrompt) {
		OnSetMessageString(AFX_IDS_IDLEMESSAGE);
		m_bResetPrompt = FALSE;
...
LRESULT CDialogWhria::OnKickIdle(WPARAM, LPARAM lCount)
{
	if (m_bStatusBar) m_CSBCtrlStatus.SendMessage(WM_IDLEUPDATECMDUI);
	if (m_bToolBar) m_CSToolBar.SendMessage(WM_IDLEUPDATECMDUI,TRUE);
...
void CDialogWhria::OnTimer(UINT nIDEvent) 
{
...
void CDialogWhria::OnSize(UINT nType, int cx, int cy) 
{	
