...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen f? Nachrichten CACListWnd 
...
//		if(m_Str2.IsEmpty())
		if(m_Str2.GetLength()<m_iFindLimit)
			{
			ShowWindow(false);
			return -1;
			}
		...
		for(int i = nStartAfter+1; i < m_SearchList.GetSize(); i++)
			...
			if(m_lMode & _MODE_FIND_ALL_)
				...
				if(m_Str1.Find(m_Str2) >= 0)
					{
						temp_list.Add(m_SearchList.GetAt(i));
					}
				}
	...
	{
		m_DisplayList.Add(temp_list.GetAt(i));
	}
	*/
	m_DisplayList.Copy(temp_list);
	if (temp_list.GetCount()>=2)
	{
		for (int i=(temp_list.GetCount()-1);i>=0;i--)
		{
			for (int j=0;j<i;j++)
			{
				if (temp_list.GetAt(i)==temp_list.GetAt(j) && i!=j)
				{
