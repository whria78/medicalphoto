...
{
	ASSERT(_tcsclen(lpszToolTip) > 0 && _tcsclen(lpszToolTip) < 64);
...
UINT CTrayDialog::TrayMenuDisable(UINT nResourceID)
{
	return m_mnuTrayMenu.EnableMenuItem(nResourceID,MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
}

UINT CTrayDialog::TrayMenuEnable(UINT nResourceID)
{
	return m_mnuTrayMenu.EnableMenuItem(nResourceID,MF_BYCOMMAND | MF_ENABLED);
}
