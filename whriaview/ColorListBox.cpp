...
BEGIN_MESSAGE_MAP(CColorListBox, CExListBox)
...
//
{
	...
	fileinfo* temp=(fileinfo*)GetItemDataPtr(lpDIS->itemID);
	if (temp->bColor)
	{
		crNorm = RGB(255,0,0);		// Color information is in item data.
	}
	else
	{
		crNorm = RGB(0,0,0);		// Color information is in item data.
	}
...
//						box. Provided because CExListBox::AddString is NOT
//						a virtual function.
//
{
	return ((CExListBox*)this)->AddString(lpszItem);
}	// AddString
...
//						CExListBox::InsertString is NOT a virtual function.
//
{
	return ((CExListBox*)this)->InsertString(nIndex, lpszItem);
}	// InsertString
...
//
{
	int nItem = ((CExListBox*)this)->InsertString(nIndex,lpszItem);
	if (nItem >= 0)
		SetItemData(nItem, rgb);
	return nItem;
}	// InsertString
